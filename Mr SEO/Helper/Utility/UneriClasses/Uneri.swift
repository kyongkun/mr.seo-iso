...
open class Uneri: UIView {
    ...
    open var uneriSpeed = CGFloat(0.5) {
        didSet {
            ...
            subUneriLayer1.speed = Float(uneriSpeed)
            subUneriLayer2.speed = Float(uneriSpeed)
        }
    }
    ...
    open var mainColor = UIColor.clear {
        didSet {
            mainGradientColors = [mainColor, mainColor]
        }
    }
    open var subColor = UIColor.clear  {
        didSet {
            subGradientColors = [subColor, subColor]
        }
    }
    open var mainGradientColors = [UIColor.themeBlueColorUneri , UIColor.themeGreenColorUneri] {
        didSet {
            mainUneriLayer.colors = mainGradientColors.map { $0.cgColor }
        }
    }
    open var subGradientColors = [UIColor.themeBlueColorUneri , UIColor.themeGreenColorUneri] {
        didSet {
            subUneriLayer.colors = subGradientColors.map { $0.cgColor }
            
            subUneriLayer1.colors = subGradientColors.map { $0.cgColor }
    ...
    private lazy var subUneriLayer1: CAGradientLayer = {
        let colors = subGradientColors.isEmpty ? [subColor, subColor] : subGradientColors
        return createGradient(colors: colors, alpha: subAlpha, path: uneriPath6)
    }()
    
    private lazy var subUneriLayer2: CAGradientLayer = {
        let colors = subGradientColors.isEmpty ? [subColor, subColor] : subGradientColors
        return createGradient(colors: colors, alpha: subAlpha, path: uneriPath2)
    }()
    ...
    public convenience init(frame: CGRect, mainColor: UIColor, subColor: UIColor) {
        ...
        subUneriLayer1.colors = [self.subColor.cgColor, self.mainColor.cgColor]
        subUneriLayer2.colors = [self.subColor.cgColor, self.mainColor.cgColor]
    ...
    public convenience init(frame: CGRect, mainGradientColors: [UIColor], subGradientColors: [UIColor]) {
        ...
        subUneriLayer1.colors = self.mainGradientColors.map { $0.cgColor }
        
        subUneriLayer2.colors = self.mainGradientColors.map { $0.cgColor }
    ...
    public func resume() {
        ...
        subUneriLayer1.resumeAnim(withSpeed: uneriSpeed)
        subUneriLayer2.resumeAnim(withSpeed: uneriSpeed)
    }
    
    public func pause() {
        mainUneriLayer.pauseAnim()
        subUneriLayer.pauseAnim()
        subUneriLayer1.pauseAnim()
        subUneriLayer1.pauseAnim()
...
//        gradientLayer.endPoint = CGPoint(x: 1.0, y: 1.0
    ...
    private func addAnimationToSubUneri1() {
        let subAnim0 = createAnimation(currentPath: uneriPath6, nextPath: uneriPath7, previousAnim: nil)
        subAnim0.beginTime = 0.0
        let subAnim3 = createAnimation(currentPath: uneriPath7, nextPath: uneriPath0, previousAnim: subAnim0)
        let subAnim4 = createAnimation(currentPath: uneriPath0, nextPath: uneriPath1, previousAnim: subAnim3)
        let subAnim5 = createAnimation(currentPath: uneriPath1, nextPath: uneriPath2, previousAnim: subAnim4)
        let subAnim6 = createAnimation(currentPath: uneriPath2, nextPath: uneriPath3, previousAnim: subAnim5)
        let subAnim7 = createAnimation(currentPath: uneriPath3, nextPath: uneriPath4, previousAnim: subAnim6)
        
        let subAnim1 = createAnimation(currentPath: uneriPath4, nextPath: uneriPath5, previousAnim: subAnim7)

        let subAnim2 = createAnimation(currentPath: uneriPath5, nextPath: uneriPath6, previousAnim: subAnim1)

        
        let subAnimGroup = createGroupAnimation(from: [subAnim0, subAnim3, subAnim4, subAnim5, subAnim6, subAnim7 , subAnim1, subAnim2])
        
        subUneriLayer1.mask?.add(subAnimGroup, forKey: UneriAnimationKey.sub.rawValue)
        
    }
    
    private func addAnimationToSubUneri2() {
        let subAnim0 = createAnimation(currentPath: uneriPath2, nextPath: uneriPath3, previousAnim: nil)
        subAnim0.beginTime = 0.0
      
        let subAnim7 = createAnimation(currentPath: uneriPath3, nextPath: uneriPath4, previousAnim: subAnim0)
        
        let subAnim1 = createAnimation(currentPath: uneriPath5, nextPath: uneriPath6, previousAnim: subAnim7)
        ...
        let subAnimGroup = createGroupAnimation(from: [subAnim0,subAnim7,subAnim1, subAnim2, subAnim3, subAnim4, subAnim5, subAnim6])
        
        subUneriLayer2.mask?.add(subAnimGroup, forKey: UneriAnimationKey.sub.rawValue)
    ...
    private func reconfigureUneri() {
        ...
        subUneriLayer1.mask?.removeAnimation(forKey: UneriAnimationKey.sub.rawValue)

        subUneriLayer2.mask?.removeAnimation(forKey: UneriAnimationKey.sub.rawValue)

        addAnimationToMainUneri()
        addAnimationToSubUneri()
        addAnimationToSubUneri1()
//        addAnimationToSubUneri2()
    }
...
//        addAnimationToSubUneri2()
        ...
        layer.addSublayer(subUneriLayer1)
        layer.addSublayer(subUneriLayer2)
